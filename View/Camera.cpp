...
#include "Library/View/View3d.h"
#include "Library/View/CamView3d.h"
#include "Library/View/SideView3d.h"
#include "Library/View/TopView3d.h"
#include "Library/View/TrajView.h"
/******************************************************************************/
...
void Camera::setup(View3d& view, double width, double height) {
  ...
  gluLookAt(-20, 0, -5, mViewpoint[0],
    mViewpoint[1], mViewpoint[2], 0.0, 0.0, 1.0);
}


void Camera::setup(CamView3d& view, double width, double height) {
  double aspect = width / height;
  double near = mRange[0];
  double far = mRange[1];
  double fov = 45.0 * M_PI / 180.0;
  ...
  glFrustum(left, right, bottom, top, near, far);
  glMatrixMode(GL_MODELVIEW);
  //qDebug("pos is %f, %f and %f; view is %f, %f and %f",mPosition[0],mPosition[1],mPosition[2],mViewpoint[0],mViewpoint[1],mViewpoint[2]);
  // pos is -20.000000, 0.000000 and 0.000000; view is 0.000000, 0.000000 and 0.000000
  //gluLookAt(0, 0,2, -4,0, -2, 0, 0, 1);
  gluLookAt(-10, 0, 10,
         mViewpoint[0], mViewpoint[1], mViewpoint[2],
         0,0,1);
  ///gluLookAt(-5,0,10, 0, 0, 0, 0, 0, 1);
}


void Camera::setup(SideView3d& view, double width, double height) {
  double aspect = width / height;
  double near = mRange[0];
  double far = mRange[1];
  double fov = 45.0 * M_PI / 180.0;
  ...
  glFrustum(left, right, bottom, top, near, far);
  glMatrixMode(GL_MODELVIEW);
  gluLookAt(0, -10, 0, mViewpoint[0],mViewpoint[1], mViewpoint[2], -1, 1, 1);
}


void Camera::setup(TrajView& view, double width, double height) {
  double aspect = width / height;
  double near = mRange[0];
  double far = mRange[1];
  double fov = 45.0 * M_PI / 180.0;
...
void Camera::setup(TopView3d& view, double width, double height) {
  double aspect = width / height;
  double near = mRange[0];
  double far = mRange[1];
  double fov = 45.0 * M_PI / 180.0;
  ...
  glMatrixMode(GL_PROJECTION);
  glFrustum(left, right, bottom, top, near, far);
  //qDebug("%f %f %f %f %f %f",left,right,bottom,top,near,far);
  glMatrixMode(GL_MODELVIEW);
  gluLookAt(-20, 0, 20, mViewpoint[0],
    mViewpoint[1], mViewpoint[2], 0.0, 0.0, 1.0);
}
