 ...
 *                 by the data
 */
Kdtree::Kdtree(const vector<kPoint>& points){
    // initialize data
 ...
 *
 */
void Kdtree::k_closest_points(const kPoint& Xq, int k, vector<int>& idxs, vector<double>& distances){
 ...
 */
void Kdtree::knn_search( const kPoint& Xq, int nodeIdx/*=0*/, int dim/*=0*/){
...
void Kdtree::closest_point(const kPoint &p, int& idx, double& dist){
    vector<int> idxs;
    vector<double> dsts;
...
int Kdtree::closest_point(const kPoint &p){
    int idx;
    double dist;
 ...
 */
bool Kdtree::ball_within_bounds(const kPoint& Xq){
 ...
 *
 */
double Kdtree::bounds_overlap_ball(const kPoint& Xq){
 ...
 */
void Kdtree::ball_query( const kPoint& point, const double radius, vector<int>& idxsInRange, vector<double>& distances ){
 ...
 */
void Kdtree::ball_bbox_query(int nodeIdx, kPoint& pmin, kPoint& pmax, vector<int>& inrange_idxs, vector<double>& distances, const kPoint& point, const double& radiusSquared, int dim/*=0*/){
    Node* node = nodesPtrs[nodeIdx];
 ...
 *
 */
void Kdtree::range_query( const kPoint& pmin, const kPoint& pmax, vector<int>& inrange_idxs, int nodeIdx/*=0*/, int dim/*=0*/ ){
    Node* node = nodesPtrs[nodeIdx];
 ...
 */
bool Kdtree::lies_in_range( const kPoint& p, const kPoint& pMin, const kPoint& pMax ){
    for (int dim=0; dim < ndim; dim++)
